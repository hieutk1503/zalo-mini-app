...
export default function AdminNews() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await axios.patch(`${API_URL}/news/${editingId}`, data, {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      <div className="p-6 border-b border-gray-200 flex justify-between items-center">
        <div>
          <h2 className="text-lg font-semibold text-gray-800">Quản lý Tin Tức</h2>
          <p className="text-sm text-gray-500 mt-1">Thêm, sửa, xóa các bản tin trên ứng dụng</p>
        </div>
        <button 
            ...
            {newsList.length === 0 ? (
              ...
              <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                <td className="p-4 align-top">
                  {item.thumbnail ? (
                    <img src={item.thumbnail} alt="" className="w-16 h-16 object-cover rounded-md" />
                  ) : (
                    <div className="w-16 h-16 bg-gray-200 rounded-md flex items-center justify-center text-gray-400 text-xs">No img</div>
                  )}
                </td>
                <td className="p-4 align-top">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
              <h3 className="text-lg font-bold">{editingId ? 'Sửa Tin Tức' : 'Thêm Tin Tức'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Đường dẫn ảnh (Thumbnail URL)</label>
                <input 
